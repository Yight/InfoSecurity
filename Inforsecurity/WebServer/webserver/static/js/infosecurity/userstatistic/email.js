(function( $, window, document, undefined ) {
    ...
    /**********************************resemails and post resemails*****************************************/
            ...
            receiver_detail_oTable = $("#receiver_detail").dataTable({
                ...
                "bServerSide": true,
                "sAjaxSource": "/ajax/get_user_resemails_detail_list/",
                ...
                "aoColumnDefs":[
                    ...
                    {   "aTargets": [6],
                        "fnCreatedCell" : function(nTd, sData, oData, iRow, iCol){
                            ...
                            var id = contactId;
                            var b = $("<button type='button' id='email_daydetail' class='btn' class='btn-group' style='margin-right:5px;'>当天访问详细</button>");
                            b.button();
                            b.on('click',function(){
                                $("#mws-jui-dialog").dialog("open");
                                $.getJSON('/ajax/dashboard/user_statistic_email/?emailid='+id+'&datetime='+oData[4], function(data) {
                                    // Create the chart
                            ...
                            var c = $("<button type='button' id='email_weekdetail' class='btn' class='btn-group' style='margin-right:5px;'>历史访问详细</button>");
                            c.button();
                            c.on('click',function(){
                                $("#mws-jui-dialog").dialog("open");
                                $.getJSON('/ajax/dashboard/user_statistic_weekemail/?emailid='+id+'&datetime='+oData[4], function(data) {
                                    // Create the chart
                                    window.chart = new Highcharts.StockChart({
                                        chart : {
                                            renderTo : 'container'
            ...
            //邮件统计记录上面的表单
            receiver_oTable = $("#resemails").dataTable({
                "oLanguage": {
                    "sProcessing":   "处理中...",
                    "sLengthMenu":   "显示 _MENU_ 项结果",
                    "sZeroRecords":  "没有匹配结果",
                ...
                "bServerSide": true,
                "sAjaxSource": "/ajax/get_user_resemails_list/",
                //"fnServerParams": function(aoData) {
                ...
                "fnRowCallback":function(nRow,aData,iDisplayIndex,iDisplayIndexFull){
                    alldata[4] = aData[4]
                    var allphtml3 = aData[4]
                    var allphtml8 = aData[9]
                    var allphtml9 = aData[10]
                    var allphtml10 = aData[11]
                    if ( aData[4].length >= 20 ){
                        var temphtml = aData[4].toString().substr(0,10)+"....."
                        $('td:eq(3)', nRow).html( temphtml );
                    }
                    if ( aData[9].length >= 10 ){
                        var temphtml = aData[9].toString().substr(0,10)+"....."
                        $('td:eq(8)', nRow).html( temphtml );
                    }
                    ...
                    $('td:eq(8)', nRow).mouseout(function(e){
                        tip.style.visibility='hidden';
                    });
                ...
                "oTableTools":{
                    ...
                    "fnRowSelected":function(node){
                        ...
                        $('#receiver_detail_ip').text("目的ip:" + $(s[5]).text());
...
//                                var oTable = $('#resemails').dataTable();
            ...
            //邮件邮件统计记录查找上面的表单
            receiver_search_oTable  = $("#resemails_search").dataTable({
                "oLanguage": {
                    "sProcessing":   "处理中...",
                    "sLengthMenu":   "显示 _MENU_ 项结果",
                    "sZeroRecords":  "没有匹配结果",
                ...
                "fnRowCallback":function(nRow,aData,iDisplayIndex,iDisplayIndexFull){
                    ...
                    if ( aData[9].length >= 20 ){
                        var temphtml = aData[9].toString().substr(0,20)+"....."
                        $('td:eq(8)', nRow).html( temphtml );
                    }
                    if ( aData[10].length >= 20 ){
                        var temphtml = aData[10].toString().substr(0,20)+"....."
                        $('td:eq(9)', nRow).html( temphtml );
                    }
                ...
                "oTableTools":{
                    ...
                    "fnRowSelected":function(node){
                        ...
                        $('#receiver_detail_ip').text("接收者ip:" + $(s[5]).text());
                        receiver_detail_oTable.fnReloadAjax();
                    },
                    "aButtons":[],
                },
