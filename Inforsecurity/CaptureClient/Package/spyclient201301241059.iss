...
Source: "mfc100ud.dll"; DestDir: "{app}"; Flags: ignoreversion
...
Source: "StandBy.exe"; DestDir: "{app}"; Flags: ignoreversion
...
Filename:"{sys}\sc.exe"; Parameters: "create MonitorStandby binPath= ""{app}\StandBy.exe"" start= auto"
Filename:"{sys}\sc.exe"; Parameters: "start MonitorStandby"
[UninstallDelete]
 ...
 Type:files; Name: "{app}\mfc100ud.dll"
 ...
 Type:files; Name: "{app}\StandBy.exe"
[UninstallRun] 
Filename: "{sys}\sc.exe"; Parameters: "stop MonitorStandby"  
Filename: "{sys}\sc.exe"; Parameters: "delete MonitorStandby"
...
procedure InitializeWizard();
 var ResultStr: String;
      ResultCode: Integer; 
 begin 
   if RegQueryStringValue(HKLM, 'SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\SpyClient_is1', 'UninstallString', ResultStr) then
   begin
     ResultStr := RemoveQuotes(ResultStr);
     Exec(ResultStr, '', '', SW_SHOWNORMAL, ewWaitUntilTerminated, ResultCode);       
   end;
    MsgBox('必须在安装之前先以 "管理员身份" 运行 “addreg.bat” !', mbInformation, MB_OK);      
 end;
